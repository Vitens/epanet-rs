...
pub extern "C" fn EN_addpattern(ph: *mut Project, id: *const c_char) -> ErrorCode {
  ...
  let result = simulation.network.add_pattern(pattern_id, &PatternData { multipliers: vec![1.0], });
...
#[unsafe(no_mangle)]
pub extern "C" fn EN_getaveragepatternvalue(ph: *mut Project, index: c_int, out_value: *mut c_double) -> ErrorCode {
...
#[unsafe(no_mangle)]
pub extern "C" fn EN_getpatternid(ph: *mut Project, index: c_int, out_id: *mut c_char) -> ErrorCode {
  let simulation = get_simulation!(ph);
...
pub extern "C" fn EN_setpatternid(ph: *mut Project, index: c_int, id: *const c_char) -> ErrorCode {
  ...
  simulation.network.pattern_map.insert(new_pattern_id.into(), index as usize);
...
#[unsafe(no_mangle)]
pub extern "C" fn EN_getpatternindex(ph: *mut Project, id: *const c_char, out_index: *mut c_int) -> ErrorCode {
  let simulation = get_simulation!(ph);
...
#[unsafe(no_mangle)]
pub extern "C" fn EN_getpatternlen(ph: *mut Project, index: c_int, out_count: *mut c_int) -> ErrorCode {
  let simulation = get_simulation!(ph);
...
#[unsafe(no_mangle)]
pub extern "C" fn EN_getpatternvalue(ph: *mut Project, index: c_int, time: c_int, out_value: *mut c_double) -> ErrorCode {
  let simulation = get_simulation!(ph);
...
#[unsafe(no_mangle)]
pub extern "C" fn EN_setpattern(ph: *mut Project, index: c_int, multipliers: *const c_double, count: c_int) -> ErrorCode {
  let simulation = get_simulation_mut!(ph);
  ...
  pattern.multipliers = unsafe { std::slice::from_raw_parts(multipliers, count as usize).to_vec() };
  ErrorCode::Ok
}
