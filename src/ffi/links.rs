...
use crate::model::network::modify::{PipeData, ValveData, PumpData};
...
#[unsafe(no_mangle)]
pub extern "C" fn EN_setlinknodes(ph: *mut Project, index: c_int, start_node: c_int, end_node: c_int) -> ErrorCode {
  ...
  match simulation.network.update_link(&link_id, &LinkUpdate { start_node: Some(start_node_id.into()), end_node: Some(end_node_id.into()), ..Default::default() }) {
    Ok(_) => ErrorCode::Ok,
  ...
  let value = match property {
    ...
    LinkProperty::HeadLoss => {
      if let Some(state) = simulation.solved_state() {
        ...
        match &link.link_type {
          LinkType::Pipe(_) | LinkType::Valve(_) => headloss.abs() * options.unit_system.per_feet(),
...
pub extern "C" fn EN_setlinkvalue(ph: *mut Project, index: c_int, property: c_int, value: c_double) -> ErrorCode {
  ...
  let result = match property {
    ...
    LinkProperty::GpvCurve | LinkProperty::PcvCurve => {
      ...
      simulation.network.update_valve(&link_id, &ValveUpdate { curve_id: Some(Some(curve_id.into())), ..Default::default() })
    }
...
#[unsafe(no_mangle)]
pub extern "C" fn EN_addlink(ph: *mut Project, id: *const c_char, link_type: c_int, start_node: *const c_char, end_node: *const c_char, out_index: *mut c_int) -> ErrorCode {
  ...
  match response {
    Ok(_) => {
      unsafe { *out_index = (*simulation.network.link_map.get(link_id).unwrap()+1) as c_int };
      ErrorCode::Ok
    }
    Err(_) => ErrorCode::InvalidParameterCode,
  }
...
#[unsafe(no_mangle)]
pub extern "C" fn EN_getheadcurveindex(ph: *mut Project, index: c_int, out_index: *mut c_int) -> ErrorCode {
  let simulation = get_simulation!(ph);
...
#[unsafe(no_mangle)]
pub extern "C" fn EN_setheadcurveindex(ph: *mut Project, index: c_int, head_curve_index: c_int) -> ErrorCode {
  let simulation = get_simulation_mut!(ph);
  ...
  let result = match link_type {
    LinkType::Pump(_) => {
      if head_curve_index == 0 {
        ...
        simulation.network.update_pump(&link_id, &PumpUpdate { head_curve_id: Some(None), ..Default::default() })
      } else {
        // set the head curve
        let head_curve_id = match simulation.network.curves.get((head_curve_index - 1) as usize) {
          Some(curve) => curve.id.clone(),
          None => return ErrorCode::UndefinedCurve,
        };

        simulation.network.update_pump(&link_id, &PumpUpdate { head_curve_id: Some(Some(head_curve_id.into())), ..Default::default() })
      }
    },
    _ => return ErrorCode::UndefinedPump,
  };
