...
impl SolverResult {
    pub fn new(n_links: usize, n_nodes: usize, n_steps: usize) -> Self {
        Self {
            flows: vec![vec![0.0; n_links]; n_steps],
            heads: vec![vec![0.0; n_nodes]; n_steps],
            demands: vec![vec![0.0; n_nodes]; n_steps],
        }
    }
    ...
    pub fn convert_units(&mut self, to_flow_units: &FlowUnits, to_unit_system: &UnitSystem) {
        ...
        self.flows
            .iter_mut()
            .flatten()
            .for_each(|flow| *flow *= flow_scale);
        self.heads
            .iter_mut()
            .flatten()
            .for_each(|head| *head *= head_scale);
        self.demands
            .iter_mut()
            .flatten()
            .for_each(|demand| *demand *= flow_scale);
    }
}
